...
Record the tasks emitted by a pipeline run.
...
### Metadata Keys by Workflow

| Workflow | Key Metadata |
| --- | --- |
| `ExactDeduplicationWorkflow` | `total_time`, `num_duplicates`, `identification_time`, `input_filegroups_time`, `id_generator_path` |
| `FuzzyDeduplicationWorkflow` | `total_time`, `num_duplicates`, `minhash_time`, `lsh_time`, `connected_components_pipeline_time`, `id_generator_path` |
| `SemanticDeduplicationWorkflow` | `total_time`, `num_duplicates`, `kmeans_time`, `pairwise_time` |
| `TextDuplicatesRemovalWorkflow` | `total_time`, `num_duplicates_removed` |
| `TextSemanticDeduplicationWorkflow` | `total_time`, `num_duplicates`, `num_duplicates_removed`, `embedding_time`, `identification_time`, `removal_time`, `kmeans_time`, `pairwise_time`, `embeddings_path`, `semantic_dedup_path`, `final_output_path`, `id_generator_path` |

<Note>
`TaskPerfUtils.collect_stage_metrics()` and `TaskPerfUtils.aggregate_task_metrics()` also accept `WorkflowRunResult` directly, in addition to `list[Task]`.
</Note>
...
Abstract base class for deduplication workflows. Enforces a consistent `run()` interface that returns `WorkflowRunResult`. Most deduplication workflows inherit from `WorkflowBase`; `TextSemanticDeduplicationWorkflow` implements the same interface as a dataclass without formal inheritance.

### Import

```python
from nemo_curator.pipeline.workflow import WorkflowBase
```
...
- [WorkflowRunResult / WorkflowBase](https://github.com/NVIDIA-NeMo/Curator/blob/main/nemo_curator/pipeline/workflow.py) {/* File added in PR #1275 */}
