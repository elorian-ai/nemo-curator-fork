...
# result.metadata contains: total_time, num_duplicates, identification_time
```
...
# result.metadata contains: total_time, num_duplicates, minhash_time, lsh_time, connected_components_pipeline_time
```
...
Available metadata varies by workflow. Common keys include `total_time` and `num_duplicates`. See the [WorkflowRunResult API reference](/api-reference/pipeline#workflowrunresult) for the complete list.
